...
public class Graph {
    ...
    List <Graph.Head> minHeads = new ArrayList<>();
    public static List<Thread> threads = new ArrayList<>();

    Graph(String path) {
        this.graph = new ConvertFile().getTreeToFile(path);
    }
    void setMinHeads (List <Graph.Head> minHeads)
    {
        this.minHeads = minHeads;
    }
    void addMinHeads (List <Graph.Head> minHeads)
    {
    ...
    public String toString() {
        ...
        sb.append("\nSum weight: ").append(getWeightGraph()).append(";");
        return sb.toString();
    }
    ...
    private class MinUFinder extends Thread {
        Head minVa = null;
        int min = Integer.MAX_VALUE;
        List <Graph.Head> Heads = new ArrayList<>();
        ...
        public void run() {
            for (Map.Entry<String, Integer> value : entry.getValue().entrySet()) {
                if (value.getValue() < min) {
                    ...
                    minVa = new Graph.Head(entry.getKey(), value.getKey(), value.getValue());
                }
            }
            try {
    ...
    public void prima()
    {
        ...
        while(!this.graph.isEmpty()) {
            ...
            if (min != null) {
                ...
                System.out.println(min.firstV+ " -> "+min.secondV+ " {"+min.valueU+"}");
